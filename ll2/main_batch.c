...
void
debugBlock(CFormat *b,size_t siz,char * title) {
	for (int i=0;i<50;i++) {
		printf("debug %s - left=%f\n",title,(*(b+i)).left);
		printf("debug %s - right=%f\n",title,(*(b+i)).right);
	}
}
...
scheduleTaskPair(convoTask* ct[], int h, int bufcounter, int j, int i) {
	...
	if (verbose_flag) printf("STP: h=%d, bufcounter=%d, lev-j=%d, pri-i=%d \n",h,bufcounter,j,i);	
...
void accCallback(void * data, size_t elementSize, void * dest, float scale) {
	CFormat * cfData=(CFormat*)data;
	CFormat * cfDest=(CFormat*)dest;
	int i=0;
	while (1) {
		cfDest[i].left+=(cfData[i].left*scale);
		cfDest[i].right+=(cfData[i].right*scale);	
...
paTestCallback( 
	...
	float scale2=1.0/((float)strPpKernel->nBlocksRoot*128.0);

	int accCounter=gspacc_read(pGspAcc, bufcounter, &accCallback, outputBuffer, scale2);
...
prepareKernel(
	 ...
	 *
	 * k=2 becauase h0=2N h1=N h2=N h3=2N h4=2N h5=4N h6=4N
	 */

	int i=0; long int k=2; int fBlock=0;
	while (1) {
		k+=2*(pow(2,i)); // each iteration deals with a pair of kernel blocks at increasing powers of two.
		if (k>nKernelBlocks) break; 
		else i++;
	}

	fBlock=(2*i)+2; // nBlocksRoot. 
	...
	ppKernel->pFilterBlockSrc[fBlock]=ppKernel->ppKernel;
	ppKernel->blocksPerSegment[fBlock]=2;
	...
	while (1) {
		...
		for (int p=1;p<3;p++) {
			...
			if (verbose_flag) 
				printf("Kernel Pointers: h=%u,start=%lu,length=%u\n",fBlock,k,fbLen); 
	...
	for (int ri=1;ri<=fBlock;ri++) {
		...
		if (verbose_flag) printf("done planning ri=%u - filter\n",ri);

		memset((void*)ppKernel->pFilterFFTW[ri],0,n[0]);
		if (verbose_flag) printf("executing filter plan ri=%u \n",ri);
		fftwf_execute(ppKernel->filterFFTBlockPlans[ri]);
		if (verbose_flag) printf("done executing filter plan ri=%u \n",ri);
...
main(int argc,char** argv) {
	...
	*/
	SNDFILE	*infile_fir1;
	SNDFILE	*infile_input;
	SNDFILE	*infile_output;
	...
	*/
	...
	const char	*fir1 = szFilter;

	/* The SF_INFO struct must be initialized before using it.
	*/
	memset (&sfinfo_fir1, 0, sizeof (sfinfo_fir1)) ;
	memset (&sfinfo_input, 0, sizeof (sfinfo_input)) ;
	memset (&sfinfo_output, 0, sizeof (sfinfo_output)) ;
	...
	if (! (infile_input = sf_open (szInput, SFM_READ, &sfinfo_input)))
	{	/* Open failed so print an error message. */
	...
	if (verbose_flag)
		;//sfinfo_print(&sfinfo_fir1,"FIR 1");
	if (sfinfo_input.channels != 2)
	{	printf ("INPUT file needs 2 channels\n") ;
		return 1 ;
	} ;
	
	memcpy(&sfinfo_output,&sfinfo_input,sizeof(SF_INFO));
	
	if (! (infile_output = sf_open (szOutput, SFM_WRITE, &sfinfo_output)))
	{	/* Open failed so print an error message. */
	...
	pInput=(CFormat*)malloc(sizeof(CFormat) * FRAMESPERBLOCK);
	pOutput=(CFormat*)malloc(sizeof(CFormat) * FRAMESPERBLOCK);
	int i=0;
	while (1) {
		int foo=sf_readf_float(infile_input,(float*)pInput,FRAMESPERBLOCK);
		if (foo<FRAMESPERBLOCK) {
			for (;foo<FRAMESPERBLOCK;foo++) {
				(*(pInput+foo)).left=0.0f;
				(*(pInput+foo)).right=0.0f;
			}
			if (i++>strPpKernel->nBlocks) break;
		}
		paTestCallback( pInput, pOutput, FRAMESPERBLOCK, NULL, 0,NULL );
		int foo1=sf_writef_float(infile_output,(float*)pOutput,FRAMESPERBLOCK);
	}

	sf_close(infile_input);
	sf_close(infile_output);
	free(pInput);
	free(pOutput);
	//abort();
